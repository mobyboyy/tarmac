...
impl RobloxApiClient {
    ...
    /// asset's name is inappropriate. The asset's name will be replaced with a
    /// generic known-good string.
    pub fn upload_model_with_moderation_retry(
        &mut self,
        ...
        if response.success {
            ...
            if message.contains("inappropriate") {
                log::warn!(
                    "Model name '{}' was moderated, retrying with different name...",
                    data.name
                );

                let new_data = ModelUploadData {
                    name: "model",
                    ..data
                };

                return self.upload_model(new_data);
            } else {
                Err(RobloxApiError::ApiError { message })
            }
        }
    ...
    fn upload_image_raw(
        ...
        let mut url = "https://data.roblox.com/data/upload/json?assetTypeId=10".to_owned();
